#!/usr/bin/env python3
# =====================================================================
#  whisperx_batch.py · 2025-07-31
#  • Transcribe one file **or** every *.wav in a directory
#  • Optional diarization (fixed N speakers or min/max range)
#  • Prints per-file timing + audio length
# ---------------------------------------------------------------------
#  Examples
#  --------
#  # Single file, diarize with exactly 2 speakers
#  python whisperx_batch.py --input audio.wav --output out --diarize --num_speakers 2
#
#  # Whole directory, no diarization
#  python whisperx_batch.py --input wav_audio --output out --no-diarize
# =====================================================================

import argparse, os, gc, time, logging
...
def load_models(device: str, compute_type: str, need_diarization: bool,
                num_spk: Optional[int], min_spk: Optional[int],
                max_spk: Optional[int]):
    """Load WhisperX and (optionally) diarization models once."""
...
def transcribe_file(
    ...
    stats = {"file": wav_path.name, "duration_s": 0.0,
             "transcribe_s": 0.0, "diarize_s": 0.0}

    # ---------- load audio ----------
    audio = whisperx.load_audio(str(wav_path))
    stats["duration_s"] = len(audio) / 16000.0  # whisperx loads @16 kHz

    # ---------- ASR ----------
    t0 = time.perf_counter()
    result = asr_model.transcribe(audio, batch_size=batch_size, 
        print_progress=True,     # <- restore progress bar
        language="en",    
    )
...
def transcribe_path(
    ...
    asr_model, diar_model = load_models(
        device, compute_type, diarize, num_spk, min_spk, max_spk
    )
    ...
    for idx, wav in enumerate(wav_paths, 1):
        ...
        stats = transcribe_file(
            ...
            asr_model,
            diar_model,
            out_raw_dir / f"{wav.stem}.txt",
            (out_diar_dir / f"{wav.stem}.txt") if diarize else None,
            batch_size,
        )
        logging.info(
            "   audio duration %.1fs | transcribe %.1fs | align %.1fs | diarize %.1fs",
            stats["duration_s"],
            stats["transcribe_s"],
            stats["align_s"],
            stats["diarize_s"],
...
def main():
    ...
    p.add_argument("--output", default=Path("transcriptions"), type=Path,
    ...
    spk.add_argument("--num_speakers", type=int, default=None,
